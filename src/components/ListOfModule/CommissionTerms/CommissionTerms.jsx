...
const handleConfirmDataClose = () => {
  ...
  const getData = async () => {
    await axios.get(`${config.baseUrl}/commissionterm/`).then((res) => {
      setloading(false);
      setPaymentterm(
        res.data.map((row) => ({
          Key:row.id,
          Terms: row.terms,
          Commission_Percrntage:row.commission_percrntage
          // Days: row.days,
  ...
  const handleFormSubmit = () => {
    axios
      .post(
        `${config.baseUrl}/commissionterm/`,
        {
         
          terms: formData.terms,
          commission_percrntage: formData.commission_percrntage,
...
const deleteUser = (record)=>
{
  ...
  axios
  .delete(
    `${config.baseUrl}/commissionterm/${record.id}/`);
       getData();
       console.log(paymentterm)
}
  ...
  const dataSource = paymentterm.map((payments) => ({
    ...
    commission_percrntage:payments.Commission_Percrntage,
    // days: payments.Days,
  ...
  const columnsData = [
    ...
    },
    {
      ...
      dataIndex: "commission_percrntage",
      key: "commission_percrntage",
      resizable: true,
      width:"26%",
      align: "left",
        sorter:(record1, record2)=>
  ...
  return (
    <div className="module-data">
      <Page_heading parent={"List of Modules"} subchild={(<Link exact to= "/module">{"Module"}</Link>)} child={"Commission Terms"} />

      <div className="module-table-container">
      <div className="filter-searchbar-container">
        <FilterAndSearchBar
          columns={columnsData}
          addBtnName={"Commission Terms"}
          onClick={showModal}
          onData={handleData}
        />
         ...
         loading={{indicator : <div><Spin/></div>, spinning:loading}}
          ...
          pagination={!loading &&{
            ...
            showTotal: (total, range) =>
              `Showing ${range[1]}-${range[1]} of ${total} Commission Terms`,
          }}
          rowClassName={(record) =>
